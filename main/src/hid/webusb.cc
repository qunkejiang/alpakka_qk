...
bool webusb_flush() {
    ...
    if (webusb_pending_empty) {
        ...
        int remaining =webusb_ptr_in - webusb_ptr_out;
        uint8_t len = remaining < 0 ? 0 : (remaining > CTRL_MAX_PAYLOAD_SIZE ? CTRL_MAX_PAYLOAD_SIZE : remaining);
        uint8_t *offset_ptr = (uint8_t*)&webusb_buffer[webusb_ptr_out];
...
void webusb_handle_status_get() {
    logging::debug_uart("WebUSB: Received status GET from app\n");
    webusb_pending_empty = true;
    webusb_pending_status_share = true;
}

void webusb_handle_status_set(uint8_t time[8]) {
    logging::debug_uart("WebUSB: Received status SET from app\n");
...
void webusb_handle_proc(uint8_t proc) {
    ...
    switch (proc)
    {
    case PROC_RESTART:esp_restart();break;
    case PROC_BOOTSEL:logging::info("WebUSB: Bootsel\n");break;//power_bootsel();//qk
    case PROC_CALIBRATE:logging::info("WebUSB: Calibrate\n");break;//config_calibrate();//qk
    case PROC_RESET_FACTORY:
        nvm->data_default();
        nvm->save_nvm_data();
        nvm->config_profile_default_all();
        break;
    case PROC_RESET_CONFIG:
        nvm->data_default();
        nvm->save_nvm_data();
        break;
    case PROC_RESET_PROFILES:nvm->config_profile_default_all();break;
    default:
        break;
    }
}
...
void webusb_handle_section_set(uint8_t profileIndex, uint8_t sectionIndex, uint8_t section[58]) {
    logging::debug_uart("WebUSB: Handle profile SET %i %i\n", profileIndex, sectionIndex);
    // Update profile in config.
    CtrlProfile *profile_cfg = Board::get_profile(profileIndex);
    memcpy(&profile_cfg->sections[sectionIndex], section, sizeof(CtrlSection));
    // // Update profile runtime.
    if(profileIndex == Board::get_nvm_data()->profile_index)
    {
        Board::GetInstance().profile_->load_from_config(profile_cfg);
    }
    //qk 同步配置到NVM
    //nvm->config_profile_sync(profileIndex);
...
void webusb_handle(Ctrl ctrl) {
    ...
    if (ctrl.message_type == CONFIG_GET) webusb_handle_config_get((Ctrl_cfg_type)ctrl.payload[0]);
    if (ctrl.message_type == CONFIG_SET) {
        webusb_pending_config_share = ctrl.payload[0];  // Echo / confirmation.
        ctrl_config_set(
            (Ctrl_cfg_type)ctrl.payload[0],  // Config index.
    ...
    if (ctrl.message_type == PROFILE_OVERWRITE) {
        Board::GetInstance().nvm_->profile_overwrite(ctrl.payload[0], ctrl.payload[1]);
    }
}
...
void webusb_read() {
    ...
    //打印全部信息
    // logging::debug_uart("WebUSB: Received protocol flags %d\n", ctrl.protocol_flags);
    // logging::debug_uart("WebUSB: Received device ID %d\n", ctrl.device_id);
    // logging::debug_uart("WebUSB: Received message type %d\n", ctrl.message_type);
    // logging::debug_uart("WebUSB: Received payload length %d\n", ctrl.len);
    // for (int i = 0; i < ctrl.len; i++) {
    //     logging::debug_uart("%02x ", ctrl.payload[i]);
