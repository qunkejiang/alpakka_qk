...
void Thumbstick::from_ctrl(CtrlProfile *ctrl, uint8_t index) {
    ...
    thumbstick.pin_x = index==0 ? PIN_THUMBSTICK_LX : PIN_THUMBSTICK_RX;
    thumbstick.pin_y = index==0 ? PIN_THUMBSTICK_LY : PIN_THUMBSTICK_RY;
    thumbstick.invert_x = index==0 ? false : true;
    thumbstick.invert_y = index==0 ? false : false;
    ...
    thumbstick.saturation = ctrl_thumbtick.saturation > 0 ? ctrl_thumbtick.saturation / 100.0 : 1.0;
    thumbstick.glyphstick_index = 0;

    if (ctrl_thumbtick.mode == THUMBSTICK_MODE_4DIR) {
        thumbstick.left = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_LEFT]);
        thumbstick.right = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_RIGHT]);
        thumbstick.up = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_UP]);
        thumbstick.down = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_DOWN]);
        thumbstick.push = Button_from_ctrl(PIN_PUSH,    ctrl->sections[SECTION_STICK_PUSH]);
        thumbstick.inner = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_INNER]);
        thumbstick.outer = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_OUTER]);
    }
    if (ctrl_thumbtick.mode == THUMBSTICK_MODE_8DIR) {
        ...
        thumbstick.up = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_UP]);
        thumbstick.down = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_DOWN]);
        thumbstick.ul = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_UL]);
        thumbstick.ur = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_UR]);
        thumbstick.dl = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_DL]);
        thumbstick.dr = Button_from_ctrl(PIN_VIRTUAL, ctrl->sections[SECTION_STICK_DR]);
        thumbstick.push = Button_from_ctrl(PIN_PUSH,    ctrl->sections[SECTION_STICK_PUSH]);
    }
