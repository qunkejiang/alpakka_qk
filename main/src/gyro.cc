...
bool Gyro::is_engaged() {
    if (gyro.engage == PIN_NONE) return false;
    if (gyro.engage == PIN_TOUCH_IN) return Board::GetInstance().touch_->data.touch_status;
...
void Gyro::config_x(double min, double max, Actions neg, Actions pos) {
...
void Gyro::config_y( double min, double max, Actions neg, Actions pos) {
...
void Gyro::config_z( double min, double max, Actions neg, Actions pos) {
