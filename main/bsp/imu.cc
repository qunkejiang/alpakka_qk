...
esp_err_t Imu::Icm_42688_update(Imu_Data_t *Data,float *G_off)
{
    uint8_t buffer[80];
    if(icm42688_read_register(&icm42688_spi,FIFO_COUNTH, buffer, 2) == ESP_OK)
    {
        ...
        if(fifo_count>0)
        {
            ...
            {;
                ...
                raw_data.temp=(temp/132.48f) + 25.f ;
                Data->temp = raw_data.temp;
                for(i = 0; i < 3; i++)
                {
                    raw_data.gyro.axis[i] = g[i]* (DEG_PER_LSB); //*DEG2RAD
                    raw_data.acc.axis[i] = acc[i] * G_PER_LSB ; 
                    Data->acc.axis[i] = raw_data.acc.axis[i];
