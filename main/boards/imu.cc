...
void Imu::parse_icm42688_data(const uint8_t *buffer, sensorData_t *Data)
{
    uint16_t i;
    int32_t acc[3] = {
    ...
    Data->temp=(temp/132.48f) + 25.f ;
    for(i = 0; i < 3; i++)
    {
        Data->gyro.axis[i] = g[i]* (DEG_PER_LSB*DEG2RAD) ; //- flash_config.IMUCalibrationConfig.G_off[i]
        Data->acc.axis[i] = acc[i] * G_PER_LSB ; //- flash_config.IMUCalibrationConfig.A_B[i] 
    }
}
...
esp_err_t Imu::Icm_42688_update(sensorData_t *Data)
{
    uint8_t buffer[80];
    if(icm42688_read_register(&icm42688_spi,FIFO_COUNTH, buffer, 2) == ESP_OK)
    {
        ...
        if(fifo_count>0)
        {
            ...
            {
                parse_icm42688_data(buffer, Data);
                return ESP_OK;
            }
